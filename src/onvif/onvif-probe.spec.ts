...
const config = (port: number) => {
  return {
    ...
    MULTICAST_ADDRESS: DEFAULT_CONFIG.MULTICAST_ADDRESS, // '0.0.0.0',
    FALLOUT_MS: 12000,
...
describe('onvif-probe', () => {
  ...
  it.only('should handle AMCREST - IP2M-841B', done => {
    const port = 3702 // 41242

    initTestServer(port)(amcrest)
    initSocketStream.flatMap(onvifProbe).run(config(port))
      .subscribe(res => {
        console.log(res.map(b => b.device))
        // expect(res[0].device).toEqual({
        //   name: 'Amcrest',
        //   hardware: 'IP2M-841B',
        //   location: 'china',
